...
@section('content')
    ...
    <style>
        ...
        .attachment-edit-table-wrap {
            ...
            border-radius: 14px;
            overflow: hidden;
            box-shadow: 0 4px 15px rgba(111, 90, 36, .08);
        }

        .attachments-table,
        .attachment-edit-table {
            margin-bottom: 0;
            color: #4B3F1F;
        }

        .attachments-table thead th,
        .attachment-edit-table thead th {
            background: #6F5A24;
            color: #fff;
            border-color: #C8B27A;
            font-weight: 700;
        ...
        .attachment-edit-table tbody td {
            background: #fff;
            border-color: #E5D6A5;
            vertical-align: middle;
            text-align: center;
        ...
        .attachment-edit-table tbody tr:hover td {
            background: #FFFDF7;
        }

        .attachments-table .attachment-title {
            color: #6F5A24;
            font-weight: 700;
            text-align: right;
        }
        ...
        .attachment-edit-table .attachment-box {
            margin: 0;
        }

        .attachment-edit-table textarea {
            min-width: 180px;
        }
                ...
                style="background:#d4af37;color:#2f3a1f;font-weight: 700;margin-right: 1rem;">
                    ...
                    @elseif(
                        !$isDesignsFiles &&
                            ...
                            !$isCotractorFiles &&
                            !$projectDocuments &&
                            ($type = 'projects' && in_array(Auth::user()->role_id, [1, 4, 11, 12, 7])))
                        <a href="{{ route('projects.owner-requirements.index', ['project' => $model->id]) }}"
                            class=" btn-add-file">
                ...
                <div class="card-body contract-grid">
                    ...
                    @if(
                        ...
                        <div class="contract-box">
                            ...
                            <a target="_blank"
                                href="{{ route('projects.contract.pdf', ['id' => $model->id, 'action' => 'preview']) }}"
                                class="btn btn-outline-primary btn-sm mb-1">
                                👁 {{ __('Preview') }}
                            </a>
                            <a href="{{ route('projects.contract.pdf', ['id' => $model->id, 'action' => 'download']) }}"
                                class="btn btn-success btn-sm mb-1">
                                ⬇ {{ __('Download') }}
                            </a>
                            <a target="_blank"
                                href="{{ route('projects.contract.pdf', ['id' => $model->id, 'action' => 'print']) }}"
                        ...
                        <div class="contract-box">
                            ...
                            <a target="_blank"
                                href="{{ route('projects.contract.pricing.pdf', [
                                    ...
                                    'action' => 'preview',
                                    'context' => 'pricing',
                                    'contractor' => auth()->user()->role_id == 3 ? auth()->id() : $model->contractor_id ?? null,
                                ]) }}"
                                class="btn btn-outline-primary btn-sm mb-1">
                                👁 {{ __('Preview') }}
                            </a>
                        ...
                        @php
                            $lastBatchId =
                                \App\Models\ProjectSchedule::where('project_id', $project->id)->max('batch_id') ?? 1;
                        @endphp
                        ...
                        @if(
                            ...
                            <div class="table-responsive attachments-table-wrap">
                                <table class="table attachments-table">
                                    <thead>
                                        <tr>
                                            <th style="width:60px;">#</th>
                                            <th>نوع المرفق</th>
                                            <th style="width:120px;">الحالة</th>
                                            <th style="width:340px;">الإجراءات</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($rows as $row)
                                            ...
                                            <tr>
                                                ...
                                                <td class="attachment-title">{{ $attTypes[$typeId] ?? 'File ' . $typeId }}
                                                </td>
                                                <td>
                                                    @if ($att)
                                                        <span class="badge bg-success">موجود</span>
                                                    @else
                                                        <span class="badge bg-secondary">غير موجود</span>
                                                    @endif
                                                </td>
                                                <td>
                                                    @if ($att)
                                                        <div class="attachment-table-actions">
                                                            <a target="_blank" href="{{ asset($att->web_path) }}"
                                                            ...
                                                            <a href="{{ asset($att->web_path) }}"
                                                                ...
                                                                <i class="fas fa-download"></i> تعديل
                                                            </a>
                                            ...
                                            @foreach ($rows as $row)
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="attachment-title">
                                                        {{ $attTypes[$typeId] ?? 'File ' . $typeId }}</td>
                                                    <td>
                                                        @if ($att)
                                                            <span class="badge bg-success">موجود</span>
                                                        @else
                    ...
                    @elseif($isCotractorFiles)
                            ...
                            <div class="contract-box">
                                ...
                                <a target="_blank"
                                    href="{{ route('projects.contract.hawya.pdf', ['id' => $model->id, 'action' => 'preview']) }}"
                                    class="btn btn-outline-primary btn-sm mb-1">
                                    👁 {{ __('Preview') }}
                                </a>

                                <a href="{{ route('projects.contract.hawya.pdf', ['id' => $model->id, 'action' => 'download']) }}"
                                    class="btn btn-success btn-sm mb-1">
                                    ⬇ {{ __('Download') }}
                                </a>

                                <a target="_blank"
                                    href="{{ route('projects.contract.hawya.pdf', ['id' => $model->id, 'action' => 'print']) }}"
                                    class="btn btn-warning btn-sm">
                                    🖨 {{ __('Print') }}
                                </a>
                            </div>
                            ...
                            <div class="contract-box">
                                ...
                                <a target="_blank"
                                    href="{{ route('projects.contract.site_delivery.pdf', ['id' => $model->id, 'action' => 'preview']) }}"
                                    class="btn btn-outline-primary btn-sm mb-1">
                                    👁 {{ __('Preview') }}
                                </a>

                                <a href="{{ route('projects.contract.site_delivery.pdf', ['id' => $model->id, 'action' => 'download']) }}"
                                    class="btn btn-success btn-sm mb-1">
                                    ⬇ {{ __('Download') }}
                                </a>

                                <a target="_blank"
                                    href="{{ route('projects.contract.site_delivery.pdf', ['id' => $model->id, 'action' => 'print']) }}"
                                    class="btn btn-warning btn-sm">
                                    🖨 {{ __('Print') }}
                                </a>
                            </div>
                            ...
                            <div class="contract-box">
                                ...
                                <a target="_blank"
                                    href="{{ route('projects.contract.bank.pdf', ['id' => $model->id, 'action' => 'preview']) }}"
                                    class="btn btn-outline-primary btn-sm mb-1">
                                    👁 {{ __('Preview') }}
                                </a>

                                <a href="{{ route('projects.contract.bank.pdf', ['id' => $model->id, 'action' => 'download']) }}"
                                    class="btn btn-success btn-sm mb-1">
                                    ⬇ {{ __('Download') }}
                                </a>

                                <a target="_blank"
                                    href="{{ route('projects.contract.bank.pdf', ['id' => $model->id, 'action' => 'print']) }}"
                                    class="btn btn-warning btn-sm">
                                    🖨 {{ __('Print') }}
                                </a>
                            </div>
                            ...
                            <div class="contract-box">
                                ...
                                <a target="_blank"
                                    href="{{ route('projects.contract.bank_table.pdf', ['id' => $model->id, 'action' => 'preview']) }}"
                                    class="btn btn-outline-primary btn-sm mb-1">
                                    👁 {{ __('Preview') }}
                                </a>

                                <a href="{{ route('projects.contract.bank_table.pdf', ['id' => $model->id, 'action' => 'download']) }}"
                                    class="btn btn-success btn-sm mb-1">
                                    ⬇ {{ __('Download') }}
                                </a>

                                <a target="_blank"
                                    href="{{ route('projects.contract.bank_table.pdf', ['id' => $model->id, 'action' => 'print']) }}"
                                    class="btn btn-warning btn-sm">
                                    🖨 {{ __('Print') }}
                                </a>
                            </div>
                                ...
                                @foreach ($rows as $row)
                                    ...
                                    <div class="contract-box">
                                        ...
                                        @if ($att)
                                            ...
                                            <a href="{{ asset($att->web_path) }}" class="btn btn-success btn-sm mb-1">
                                                ⬇ تعديل
                                            </a>
                        ...
                        @else
                            ...
                            @if (!$projectDocuments && !$isDesigns && !$isDesignsFiles && !$isAchievements)
                                <div class="contract-box">
                                    ...
                                    <a target="_blank"
                                        href="{{ route('projects.contract.owner_consultant.pdf', ['id' => $model->id, 'action' => 'preview']) }}"
                                        class="btn btn-outline-primary btn-sm mb-1">
                                        👁 {{ __('Preview') }}
                                    </a>
                                    <a href="{{ route('projects.contract.owner_consultant.pdf', ['id' => $model->id, 'action' => 'download']) }}"
                                        class="btn btn-success btn-sm mb-1">
                                        ⬇ {{ __('Download') }}
                                    </a>
                                    <a target="_blank"
                                        href="{{ route('projects.contract.owner_consultant.pdf', ['id' => $model->id, 'action' => 'print']) }}"
                                        class="btn btn-warning btn-sm">
                                        🖨 {{ __('Print') }}
                                    </a>
                                </div>
                                ...
                                <div class="contract-box">
                                    ...
                                    <a target="_blank"
                                        href="{{ route('projects.contract.owner-requirements.pdf', ['id' => $model->id, 'action' => 'preview']) }}"
                                        class="btn btn-outline-primary btn-sm mb-1">
                                        👁 {{ __('Preview') }}
                                    </a>

                                    <a href="{{ route('projects.contract.owner-requirements.pdf', ['id' => $model->id, 'action' => 'download']) }}"
                                        class="btn btn-success btn-sm mb-1">
                                        ⬇ {{ __('Download') }}
                                    </a>

                                    <a target="_blank"
                                        href="{{ route('projects.contract.owner-requirements.pdf', ['id' => $model->id, 'action' => 'print']) }}"
                                        class="btn btn-warning btn-sm">
                                        🖨 {{ __('Print') }}
                                    </a>
                                </div>
...
@endif
        ...
        @if (auth()->user()->role_id == 3 && request('type') == 'projects' && request('mode') == 'tender')
            <div
                style="border:2px solid #0d6efd; background:#f8fbff; padding:20px; border-radius:10px; margin-bottom:20px;">
                ...
                <ol style="line-height:1.9; padding-right:20px; font-size:17px;">
                    ...
                    <li>
                        حرصًا من مكتب سافانا على التطوير المستمر وتحسين جودة خدماته، فقد تم اعتماد نظام المناقصات عبر النظام
                        الداخلي للمكتب، وذلك بهدف تنظيم العمل وتسهيل إجراءات الاطلاع والتسعير.
                    </li>

                    <li>
                        بعد استلام المقاول اسم المستخدم وكلمة المرور الخاصة به، يمكنه الدخول إلى صفحة المناقصة عبر النظام،
                        حيث ستظهر له جميع مستندات المناقصة.
                    </li>
                    ...
                    <li>
                        يقوم المقاول بالدخول إلى مستند الكميات، ويكون مخولًا بإدخال الاسعار و الكميات فقط، بالإضافة إلى
                        إمكانية إضافة الملاحظات إن وجدت.
                    </li>

                    <li>
                        بعد الانتهاء من إدخال الأسعار والملاحظات، يقوم المقاول بحفظ الملف، وبذلك تكون عملية التسعير قد
                        اكتملت.
                    </li>
        ...
        @php
            $canUpload = in_array(auth()->user()->role_id, [1, 4, 11, 12, 7]) || auth()->id() == $model->id;
        @endphp
        ...
        @if (in_array(auth()->user()->role_id, [1, 4, 11, 12, 7]))
            ...
            @if ($canUpload)
                <form class="opacity: {{ $canUpload ? 1 : 0 }}"
                    action="{{ route('users.attachments.store', ['id' => $model->id, 'type' => $type]) }}"
        ...
        border-bottom:2px solid #C8B27A;">
                    ...
                    <button style="color:#fff;" type="button" id="addAttachment" class="btn btn-add-file d-none">
                ...
                @endforeach --}}
                    ...
                    <div class="table-responsive attachment-edit-table-wrap">
                        <table class="table attachment-edit-table">
                            <thead>
                                <tr>
                                    <th style="width:60px;">#</th>
                                    <th style="width:240px;">نوع المرفق</th>
                                    <th>الملف</th>
                                    <th style="width:180px;">تاريخ الانتهاء</th>
                                    <th style="width:260px;">الملاحظات</th>
                                    <th style="width:120px;">الإجراءات</th>
                                </tr>
                            </thead>
                            <tbody id="attachmentContainer">
                                @foreach ($rows as $i => $row)
                                    @php
                                        $att = $row['attachment'];
                                    @endphp
                                    <tr class="attachment-card">
                                        <td>{{ $loop->iteration }}</td>
                                        <td>
                                            @if ($att)
                                                <input type="hidden" name="attachments[{{ $i }}][id]"
                                                    value="{{ $att->id }}">
                                            @endif

                                            <select name="attachments[{{ $i }}][attachment_type_id]"
                                                class="form-control">
                                                @foreach ($attTypes as $id => $name)
                                                    <option value="{{ $id }}"
                                                        {{ $row['type_id'] == $id ? 'selected' : '' }}>
                                        ...
                                        <td>
                                            <div class="attachment-box border rounded p-2 small bg-light">
                                                <input type="hidden" class="delete-flag"
                                                    name="attachments[{{ $i }}][delete]" value="0">
                                                ...
                                                @if ($att)
                                                    ...
                                                    <a href="{{ asset($att->web_path) }}" target="_blank"
                                                        class="btn btn-success btn-sm preview-file stored-file mt-2">
                                                        <i class="fas fa-eye"></i> معاينة
                                                    </a>
                                                @else
                                                    ...
                                                    <a href="#" target="_blank"
                                                        class="btn btn-success btn-sm preview-file d-none mt-2">
                                                        <i class="fas fa-eye"></i> معاينة
                                                    </a>
                                                @endif
                                            </div>
                                        ...
                                        <td>
                                            <textarea name="attachments[{{ $i }}][notes]" class="form-control" rows="2">{{ $att->notes ?? '' }}</textarea>
                                        </td>
                                        <td>
                                            <button type="button" class="btn btn-danger btn-sm removeAttachment">
                    ...
                    {{-- OLD CARD VIEW: محفوظ لإعادة الاستخدام لاحقًا، ومعطل حتى لا يكرر نفس المرفقات في الإنتاج. --}}
                    @if (false)
                        @foreach ($rows as $i => $row)
                            @php
                                $att = $row['attachment'];
                            ...
                            <div class="attachment-card">
                                ...
                                <div class="attachment-item p-3">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="date" name="attachments[{{ $i }}][expiration_date]"
                                            value="{{ optional($att?->expiration_date)->format('Y-m-d') }}"
                                            class="form-control">
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <textarea name="attachments[{{ $i }}][notes]" class="form-control" rows="3">{{ $att->notes ?? '' }}</textarea>
                                    </div>
            ...
            <div
                class="opacity: {{ $canUpload ? 1 : 0 }}  buttons_container text-center mt-4 d-flex justify-content-center gap-3">

                {{-- Save --}}
                <button type="submit" name="action" value="save" class="btn btn-olive px-4">
                    💾 {{ __('حفظ') }}
                </button>

                {{-- Show ONLY when type != projects --}}
                @if ($type !== 'projects' && !$isAdminFiles)
                    <button type="submit" name="action" value="save_create_project" class="btn btn-olive px-4">
                        ➕ {{ __('Save & Create Project') }}
                    </button>
                @endif

                {{-- Cancel Button --}}
                <a href="{{ $type === 'projects' ? route('projects.index') : route('users.index') }}"
...
@push('scripts')
    ...
    <script>
        ...
        document.getElementById('addAttachment').addEventListener('click', function() {
            ...
            a href = "#"
            target = "_blank"
            class = "btn btn-success preview-file d-none" > 👁معاينة <
            /a>

            <
            button type = "button"
            class = "btn btn-danger remove-file" > 🗑حذف <
            /button>
            ...
            <
            textarea name = "attachments[${attachmentIndex}][notes]"
            class = "form-control"
            rows = "3"
